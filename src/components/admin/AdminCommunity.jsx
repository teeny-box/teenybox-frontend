...
const AdminCommunity = () => {
  ...
  const getPosts = async () => {
    ...
    try {
      const res = await fetch(`${postUrl}?page=${page}&limit=10&sortBy=${sortBy}&sortOrder=${sortOrder}`, {
        credentials: "include",
      });
  ...
  const handleDelete = async () => {
    try {
      ...
      if (res.ok) {
        const newPosts = posts.filter((post) => !checkedList.includes(post.post_number));
        setPosts(newPosts);
        setCheckedList([]);
        getPosts();
        setOpenAlert2(true);
  ...
  const handleChangeChecked = (e) => {
    if (e.target.checked) {
      ...
      setCheckedList((cur) => cur.filter((post) => post !== e.target.value));
    }
  };
  ...
  const handleAllCheck = (e) => {
    ...
    if (e.target.checked) {
      setCheckedList(allPosts.map((post) => post.post_number));
    } else {
      setCheckedList([]);
    }
  };
          ...
          {state === "loading" ? (
            ...
            <div className="my-table">
              ...
              <div className="table-body">
                {posts.map((promotion, index) => (
                  <div key={promotion.post_number}>
