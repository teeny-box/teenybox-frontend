...
const AdminPromotion = () => {
  ...
  const getPromotions = async () => {
    ...
    try {
      const res = await fetch(`${promotionUrl}?page=${page}&limit=10&sortBy=${sortBy}&sortOrder=${sortOrder}`, {
        credentials: "include",
      });
  ...
  const handleDelete = async () => {
    try {
      ...
      if (res.ok) {
        const newPromotions = promotions.filter((promotion) => !checkedList.includes(promotion.promotion_number));
  ...
  const handleChangeChecked = (e) => {
    if (e.target.checked) {
      ...
      setCheckedList((cur) => cur.filter((promotion) => promotion !== e.target.value));
    }
  };
  ...
  const handleAllCheck = (e) => {
    ...
    if (e.target.checked) {
      setCheckedList(allPromotions.map((promotion) => promotion.promotion_number));
    } else {
      setCheckedList([]);
    }
  };
      ...
      <div className="admin-board-container">
        ...
        <div className="body">
          {state === "loading" ? (
            ...
            <ServerError onClickBtn={() => getPromotions()} />
          ) : promotions.length ? (
            <div className="my-table">
              ...
              <div className="table-body">
                {promotions.map((promotion, index) => (
                  <div key={promotion.promotion_number}>
                    <div className="table-item">
                      ...
                      <div className="item-box" style={{ width: "24%" }}>
                        <p>{promotion.user.nickname}</p>
                      </div>
                      <div className="item-box" style={{ width: "20%" }}>
                        <TimeFormat time={promotion.createdAt} />
                      </div>
                      <div className="item-box" style={{ width: "22%" }}>
                        <Checkbox
                          value={promotion.promotion_number}
                          sx={{
                            "& .MuiSvgIcon-root": {
                              color: "#FFB400",
                            },
