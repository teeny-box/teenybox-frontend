...
function MyCommunityBoard({ user, setUserData }) {
  ...
  const getPosts = async () => {
    setState("loading");
    try {
      const res = await fetch(`${postUrl}/user/${user._id}?page=${page}&limit=10&sortBy=${"time"}&sortOrder=${order}`);
  ...
  const handleChangeChecked = (e) => {
    const communityNumber = Number(e.target.value);

    if (e.target.checked) {
      setCheckedList((cur) => [...cur, communityNumber]);
    } else {
      setCheckedList((cur) => cur.filter((number) => number !== communityNumber));
    }
  };
                    ...
                    >
                      ...
                      <div className="item-box" style={{ width: "22%" }}>
                        <Checkbox
                          ...
                          checked={checkedList.includes(post.post_number)}
